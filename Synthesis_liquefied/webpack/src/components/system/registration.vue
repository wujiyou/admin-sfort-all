<template>
  <div>
    <!-- <div style="background:white;width:99.1%;padding-left:15px">
      <my-bread level1="预警管理" level2="系统监管" level3="气瓶登记日志"></my-bread>
    </div> -->
...
export default {
  ...
  methods: {
    ...
    async searchUser() {
      ...
      if (dering == "") {
        ...
        const res = await this.$http.get(
          `/operation-logs/findPage?page=${this.page}&rows=${this.rows}&createdTime=${resDate}&username=${this.username}&type=1`
        );
        console.log(res);
    ...
    async getUserlist() {
      ...
      const res = await this.$http.get(
        `/operation-logs/findPage?page=${this.page}&rows=${this.rows}&username=${this.username}&createdTime=${this.createdTime}&type=1`
      );
      console.log(res);
      this.userlist = res.data.data.list;
