          ...
          <el-col :span="24">
            <el-col :span="12">
              <el-form-item class="red_s" label="设备类型" label-width="140px" prop="equType">
                ...
                <!-- <el-input v-model="form.equType" autocomplete="off"></el-input> -->
                <!-- disabled实现用户名禁止改动 -->
              </el-form-item>
            </el-col>
            <el-col :span="12">
...
export default {
  ...
  methods: {
    async Regtion() {
      ...
      if (this.form.makeData == "") {
        ...
        const res = await this.$http.get(
          `IDGenerate/IndustralCode?regId=${regId}&makeData=${data}`
        );
        console.log(res);
    ...
    uploadFile(item) {
      ...
      this.$http({
        method: "post",
        url: `/industrial/template/analysis?id=${this.ids}`,
        data: form,
        headers: {
    ...
    firstAddUserDia() {
      this.$router.push({ name: "industrial" });
    },
    // 编辑用户
    // 编辑用户发送请求
    async EditUser() {
    ...
    async showEdituser(user) {
      ...
      const res = await this.$http.get(`unitdrop/manufacturer?bottleType=2`);
      console.log(res);
    ...
    async searchUser() {
      const res = await this.$http.get(
        `/cylinder/industrial/selectIdByAll?id=${this.ids}&pageSize=${this.pageSize}&currpage=1&gasId=${this.gasId}&medium=${this.medium}`
      );
      console.log(res);
    ...
    getUserlist() {
      this.ids = this.$route.query.regId;
      console.log(this.ids);
      this.$http
        .get(
