    ...
    <div class="appContainer">
      ...
      <el-col :span="24">
        <div class="box">
          ...
          <div v-if="isMap" :id="mapId" :style="heig"></div>
        </div>
      </el-col>
    </div>
  </div>
...
import $ from "jquery";
// import liquefied from "./liquefied"
...
export default {
  ...
  mounted() {
    // var _that=this
    this.myMapChart = this.$echarts.init(document.getElementById(this.mapId));
    this.mapChart();
  },
  beforeDestroy() {},
  methods: {
    ...
    mapChart() {
      ...
      var mapdata = [];
      var _that = this;
      if(_that.provinces[_that.proviceName]===undefined){
        var pName=_that.proviceName+'市'
      }else{
        var pName=_that.proviceName

      }
     
       console.log(pName)
      // if(_that.special[pName]!== undefined){
      //   var params = `city/${_that.cityMap[pName]}.json`;
      // }else{
        var params = `province/${_that.provinces[pName]}.json`;
      // }
      var _that = this;
      $.getJSON(
        "/map/" + params, //'/static/map/province/'+_that.provinces[ _that.pro]+'.json'
        function (data) {
          var d = [];
          ...
          _that.$echarts.registerMap(pName, data);
          _that.renderMap(pName, d);
          _that.city = "";
        }
      );
    ...
    renderMap(map, data, obj2) {
      ...
      if (_that.provinces[map] !== undefined) {
        console.log("map存在");
        this.$store.commit(_that.CommitType, map);
    ...
    renderMapData(childNode) {
      ...
      let url = {
        P: `/map/province/${this.provinces[childNode.name]}.json`,
        C: `/map/city/${this.cityMap[childNode.name]}.json`,
      };
      var _that = this;
      if (childNode.type === "P" || childNode.type === "C") {
        ...
        $.getJSON(url[childNode.type], (data) => {
          ...
          console.log(localStorage.getItem("directlyCity"))
        // 调用百度地图
        let cityDeta =
          childNode.type === "X" ? JSON.parse(localStorage.getItem("city")) : JSON.parse(localStorage.getItem("directlyCity"));
        let da = cityDeta.features.findIndex((val) => {
          return childNode.name == val.properties.name;
        });
        console.log(cityDeta.features);
        console.log("-------------------");
        let gt = cityDeta.geometry[da].coordinates[0][0][0][0];
        let lt = cityDeta.geometry[da].coordinates[0][0][0][1];
        if (this.areaType == 2) {
          this.createMap(
            gt,
            lt,
            childNode.name,
            this.cityMap[childNode.parent]
          );
        } else {
            ...
            (data) => {
              data.data.forEach((value) => {
                ...
                if (value.name == childNode.name) {
                  this.$store.commit(_that.CommitType, value.code);
    ...
    back() {
      ...
      let url = {
        P: `/map/china.json`,
        C: `/map/province/${this.provinces[childNode.parent]}.json`,
        X: `/map/city/${this.cityMap[childNode.parent]}.json`,
        S: `/map/city/${this.cityMap[childNode.parent]}.json`,
      };
      if (childNode.type === "P" || childNode.type === "C") {
        console.log(childNode.parent);
        //  if(_that.provinces[childNode.parent]!==undefined){
        //    this.$store.commit("TOGDACITY1", _that.provinces[childNode.parent])
        //    console.log(childNode.parent)
        //  }else{
        console.log(childNode.parent);
        this.$store.commit(_that.CommitType, childNode.parent);
        //  }
