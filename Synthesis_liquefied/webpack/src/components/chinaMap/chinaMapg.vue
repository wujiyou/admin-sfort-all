    ...
    <div class="appContainer">
      <el-col :span="24">
        <div class="box">
          ...
          <div v-if="isMap" id="good" :style="heig"></div>
        </div>
      </el-col>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    // var _that=this
    this.myMapChart = this.$echarts.init(document.getElementById("good"));
    this.mapChart();
  },
  beforeDestroy() {},
  methods: {
    ...
    async createMap(gt, lt, cityName,cityMapCode) {
      ...
      var city = cityName
      // ?code=${city}
      const res = await this.$http.get(`/new-industry/map`);
      console.log(res);
      this.mapPoints = res.data.data;
       console.log(cityMapCode+"<----------");

      this.map = new BMap.Map("good");
      let map = this.map;
      let _that = this;
      ...
      bdary.get(cityName, function (rs) {
        ...
        for (var i = 0; i < count; i++) {
          ...
          // 设置百度地图多个标记点
          // for(var i =0;i<_that.mapPoints.length;i++){
    ...
    renderMap(map, data, obj2) {
      ...
      if (_that.provinces[map] !== undefined) {
        console.log("map存在");
        this.$store.commit("TOGDACITY1", map);
            ...
            (data) => {
              data.data.forEach((value) => {
                ...
                if (value.name == childNode.name) {
                  this.$store.commit("TOGDACITY1", value.code);
    ...
    back() {
      ...
      if (childNode.type === "P" || childNode.type === "C") {
        ...
        this.$store.commit("TOGDACITY1", childNode.parent);
