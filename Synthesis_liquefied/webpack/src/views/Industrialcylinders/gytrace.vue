...
export default {
  ...
  methods: {
    ...
    async getUserlist() {
      ...
      const res = await this.$http.get(
        `/public/gas/industrial/list?pageSize=${this.pageSize}&currPage=${this.currPage}&gunId=${this.gunId}&gasId=${this.gasId}&createTime=${this.createTime}&endTime=${this.endTime}`
      );
      // &name=${this.name}
      console.log(res);
      this.userlist = res.data.data.list;
