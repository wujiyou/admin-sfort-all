    ...
    <el-card class="box-card" style="margin:20px 20px 0 20px;">
      <el-row>
        ...
        <el-col :span="24">
          ...
          <el-col :span="12" style="color:white">1</el-col>
          <el-col :span="2">
...
export default {
  ...
  methods: {
    ...
    async AddUser() {
      ...
      const res = await this.$http.post(
        `/public/customer/orderinformation/save`,this.editUserForm
      );
      console.log(res);
    },
    showAddUserDia() {
    ...
    async searchUser() {
      const res = await this.$http.get(
        `/pc-customer-order/findPage?rows=${this.pageSize}&page=${this.page}&orderNo=${this.orderNo}&state=${this.state}`
      );
    ...
    async getUserlist(val = 0) {
      const res = await this.$http.get(
        `/pc-customer-order/findPage?rows=${this.pageSize}&page=${this.currPage}&orderNo=${this.orderNo}&state=${this.state}`
      );
      console.log(res);
      this.userlist = res.data.data.list;
      this.total = res.data.data.totalCount;
