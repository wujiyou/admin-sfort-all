<template>
  <div>
    <!-- <div style="background:white;width:99.1%;padding-left:15px">
      <my-bread level1="综合查询" level2="车用气瓶" level3="气瓶充装记录"></my-bread>
    </div> -->
    <el-card class="box-card" style="margin:20px 20px 0 20px;">
      ...
      <template>
        ...
        >
          ...
          <el-table-column show-overflow-tooltip prop="createTime" label="充装时间">
...
export default {
      ...
      async downtemplatee() {
        ...
        if (this.userlist.length === 0) {
          ...
          const res = await this.$http
            .post(
              `/public/gas/CarFilling/creat/car/record`,
              arr,
  ...
  methods: {
    ...
    async getUserlist() {
      ...
      const res = await this.$http.get(
        `/public/gas/CarFilling/CarFillingPaging?pageSize=${this.pageSize}&currPage=${this.currPage}&gunId=${this.gunId}&gasId=${this.gasId}&createTime=${this.createTime}&endTime=${this.createTime2}`
      );
      // &name=${this.name}
      console.log(res);
      this.userlist = res.data.data.list;
