<template>
  <div
    class="titleNavBox"
    :style="{width: (getCollapseState == false)?'calc(100% - 241px)':'calc(100% - 64px)'}"
  >
    <div style="position: relative">
      ...
      <div class="titleNav" @click.right.prevent="mouseRight">
        <el-tag
          ...
          @close="closeTag(item.name, item.activeIndex)"
          :closable="(item.name == '液化气瓶综合数据平台')? false : true"
        >{{item.name}}</el-tag>
      </div>
        ...
        if (nav[i].children) {
          for (let j = 0; j < nav[i].children.length; j++) {
            if (nav[i].children[j].path == path) {
              ...
              console.log(nav[i].children[j].name);
              // crumbs.push({
