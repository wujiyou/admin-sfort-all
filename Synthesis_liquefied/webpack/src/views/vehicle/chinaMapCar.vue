    ...
    <div class="appContainer">
      <el-col :span="24">
        <div class="box wuyu">
          ...
          <div v-if="isMap" id="cheditu" :style="heig"></div>
        </div>
      </el-col>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    // var _that=this
    this.myMapChart = this.$echarts.init(document.getElementById("cheditu"));
    this.mapChart();
  ...
  methods: {
    ...
    async createMap(gt, lt, cityName,cityMapCode) {
      ...
      // ?cityCode=${cityscode}&areaName=${cityNames}
       const res = await this.$http.get(`/car-stats/mapUnit`);
        console.log(res);
        this.mapPoints = res.data.data;
         

      this.map = new BMap.Map("cheditu");
      let map = this.map;
       let _that = this;
    ...
    renderMap(map, data, obj2) {
      ...
      if (_that.provinces[map] !== undefined) {
        console.log("map存在");
        console.log(map);
        this.$store.commit("TOVEHCITY1", map);
      } else if (_that.cityMap[map] !== undefined) {
        this.$store.commit("TOVEHCITY1", map);
        console.log(_that.cityMap[map]);
      }
    ...
    renderMapData(childNode) {
      ...
      if (childNode.type === "P" || childNode.type === "C") {
        ...
        this.createMap(gt, lt, childNode.name,this.cityMap[childNode.parent]);
            ...
            (data) => {
              data.data.forEach((value) => {
                ...
                if (value.name == childNode.name) {
                  this.$store.commit("TOVEHCITY1", value.code);
    ...
    back() {
      ...
      if (childNode.type === "P" || childNode.type === "C") {
        ...
        //    this.$store.commit("TOVEHCITY1", _that.provinces[childNode.parent])
        //    console.log(childNode.parent)
        //  }else{
        //    console.log(childNode.parent)
        this.$store.commit("TOVEHCITY1", childNode.parent);
        //  }
        $.ajaxSettings.async = false; //将$.getJSON改为同步
        $.getJSON(url[childNode.type], (data) => {
          // console.log(data)
        ...
        console.log(_that.cityMap[childNode.parent] + "--------------");
        this.map = null;
        this.isMap = false;
        this.$nextTick(() => {
          this.isMap = true;
          this.$nextTick(() => {
            this.myMapChart = this.$echarts.init(
              document.getElementById("cheditu")
            );
